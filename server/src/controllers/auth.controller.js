...
export const signupDoctor = asyncHandler(async (req, res) => {
   const { name, email, password, phone, hospitalName, speciality, experience } = req.body;
   if ([name, email, password, hospitalName, speciality].some(f => !f || f.trim() === "")) throw new ApiError(400, "Mandatory execution gaps.");
   
   if (await Doctor.findOne({ email })) throw new ApiError(409, "Identity overlap constraints failed");

   const doc = await Doctor.create({ name, email, password, phone, hospitalName, speciality, experience, role: ROLES.DOCTOR });
...
export const signinHospital = asyncHandler(async (req, res) => {
   ...
   if (!hosp || !(await hosp.isPasswordCorrect(password))) throw new ApiError(401, "Execution invalid locally");
