...
export default function DoctorDashboard() {
  ...
  const stats = [
    ...
    { title: "Pending Requests", value: "3", icon: Clock, color: "text-orange-600", bg: "bg-orange-100" },
    { title: "Total Patients", value: "342", icon: Users, color: "text-green-600", bg: "bg-green-100" },
  ];
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-gray-50">
              {[
                { time: "09:00 AM", name: "Alice Johnson", reason: "Fever and cold", status: "Confirmed", statusBg: "bg-blue-100 text-blue-700" },
                { time: "10:30 AM", name: "Michael Smith", reason: "Follow-up check", status: "Pending", statusBg: "bg-orange-100 text-orange-700" },
              ].map((apt, idx) => (
                <tr key={idx} className="hover:bg-gray-50 transition-colors">
                  <td className="py-4 px-4 font-medium text-gray-900">{apt.time}</td>
                  <td className="py-4 px-4">{apt.name}</td>
                  <td className="py-4 px-4 text-gray-500 truncate max-w-[200px]">{apt.reason}</td>
                  <td className="py-4 px-4">
                    <span className={`px-3 py-1 rounded-full text-xs font-semibold ${apt.statusBg}`}>
                      {apt.status}
                    </span>
                  </td>
                  <td className="py-4 px-4 text-right">
                    <button className="text-green-600 hover:text-green-800 font-medium text-sm border border-green-200 px-3 py-1.5 rounded-lg hover:bg-green-50 transition-colors">Manage</button>
                  </td>
                </tr>
              ))}
            </tbody>
